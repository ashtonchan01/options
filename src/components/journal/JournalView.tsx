...
import { derivePortfolio } from '../analytics/AnalyticsView'
...
// Single compact row — trade-performance KPIs (Net P&L, Win Rate, Profit
// Factor) alongside the account-level figures that used to live in their own
// PortfolioSummaryPanel box below (Net Liquidation, Unrealised/Realised P&L,
// Cash). One combined strip instead of two separate visual blocks; the rest
// of the old stats (Avg Win/Loss, Max DD, Open Premium, Closed, Streak) and
// the Equity Curve were dropped to make room for the Calendar embed.
function KpiStrip({ closed, state }: { closed: JournalPosition[]; state: AppState }) {
  const s = useMemo(() => computeStats(closed), [closed])
  const { netLiq, totalUnrealized, realizedPnL, cashBalance } = useMemo(() => derivePortfolio(state), [state])
  ...
  const stats = [
    ...
    { label: 'Unrealised P&L',  value: fmt$(totalUnrealized), color: pnlColor(totalUnrealized) },
    { label: 'Realised P&L',    value: fmt$(realizedPnL),     color: pnlColor(realizedPnL) },
    { label: 'Cash (Base)',     value: fmt$(cashBalance),     color: 'var(--text-1)' },
  ]
  return (
...
// KPI strip on top (see KpiStrip above for what's in it), the Calendar
// (month grid + activity list) filling the rest — replaces the old Equity
// Curve chart and the separate stats/Portfolio Summary block below it.
export function OverviewTab({ closed, state }: {
  ...
  return (
    <div className="jr-overview-compact">
      <KpiStrip closed={closed} state={state} />
