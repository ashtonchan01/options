...
function OptionRow({ r, rank }: { r: ScanResult; rank: number }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: GRID, gap: 4, alignItems: 'center', padding: '5px 0', borderBottom: '1px solid var(--border)', fontSize: 11, fontFamily: 'IBM Plex Mono, monospace' }}
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
        <span style={{ padding: '1px 6px', fontSize: 9, fontWeight: 700, background: `${color}15`, border: `1px solid ${color}40`, color, fontFamily: "'Chakra Petch', sans-serif", letterSpacing: '0.5px' }}>{label}</span>
        <span style={{ fontSize: 9, color: 'var(--text-4)', fontFamily: 'IBM Plex Mono, monospace' }}>TOP {items.length}</span>
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 3 }}>
          {flags.map(f => <span key={f} style={{ padding: '0 4px', fontSize: 8, fontWeight: 700, background: `${FLAG_COLORS[f]}12`, color: FLAG_COLORS[f], fontFamily: "'Chakra Petch', sans-serif" }}>{FLAG_LABELS[f]}</span>)}
        </div>
      </div>
      <MiniHeader />
...
const inputStyle: React.CSSProperties = { width: 54, padding: '3px 6px', fontSize: 11, textAlign: 'right', background: 'var(--bg-card)', border: '1px solid var(--border)', color: 'var(--text-1)', fontFamily: 'IBM Plex Mono, monospace', outline: 'none', borderRadius: 3 }
const labelStyle: React.CSSProperties = { display: 'flex', alignItems: 'center', gap: 5, fontSize: 10, color: 'var(--text-3)', fontFamily: 'IBM Plex Mono, monospace' }
    ...
    <div style={{ padding: 16, height: '100%', display: 'flex', flexDirection: 'column', gap: 10, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', flexShrink: 0 }}>
        ...
        <input type="text" value={tickerInput}
          ...
          placeholder="+ TICKER"
          style={{ width: 80, padding: '5px 8px', fontSize: 11, background: 'var(--bg-elevated)', border: '1px solid var(--border)', color: 'var(--text-1)', fontFamily: 'IBM Plex Mono, monospace', outline: 'none', borderRadius: 3 }}
        />

        {scanning && <span style={{ fontSize: 11, color: 'var(--accent)', fontFamily: 'IBM Plex Mono, monospace', animation: 'pulse 2s infinite' }}>{scanProgress || 'Initializing…'}</span>}

        {scanned && (
          <span style={{ fontSize: 11, color: 'var(--text-4)', marginLeft: 'auto', fontFamily: 'IBM Plex Mono, monospace' }}>
      ...
      <div style={{ display: 'flex', gap: 6, flexShrink: 0, flexWrap: 'wrap', alignItems: 'center' }}>
        <span style={{ fontSize: 9, color: 'var(--text-5)', letterSpacing: 2, fontFamily: "'Chakra Petch', sans-serif", marginRight: 2 }}>PUTHOUSE</span>
        ...
        {activeCfg && mode !== 'custom' && (
          <span style={{ marginLeft: 6, fontSize: 11, color: meta.color, fontFamily: 'IBM Plex Mono, monospace', fontWeight: 600 }}>
      ...
      {mode === 'custom' && (
        <div style={{ display: 'flex', gap: 16, alignItems: 'center', padding: '8px 14px', background: 'var(--bg-elevated)', border: '1px solid #a855f740', borderRadius: 6, flexShrink: 0, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 9, fontWeight: 700, color: '#a855f7', letterSpacing: 2, fontFamily: "'Chakra Petch', sans-serif" }}>PARAMS</span>
      ...
      {customTickers.length > 0 && (
        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', flexShrink: 0, alignItems: 'center' }}>
          <span style={{ fontSize: 9, color: 'var(--text-5)', letterSpacing: 1.5, fontFamily: "'Chakra Petch', sans-serif" }}>CUSTOM:</span>
          {customTickers.map(sym => (
            <button key={sym} onClick={() => removeTicker(sym)} title="Remove" style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '2px 8px', fontSize: 10, fontWeight: 600, background: 'var(--accent-dim)', border: '1px solid rgba(0,229,255,0.2)', color: 'var(--accent)', cursor: 'pointer', borderRadius: 3, fontFamily: 'IBM Plex Mono, monospace' }}>
      ...
      {scanning && !scanned && (
        <div style={{ textAlign: 'center', paddingTop: 60 }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--text-4)', marginTop: 6, fontFamily: 'IBM Plex Mono, monospace' }}>Parallel fetch · CBOE delayed quotes</div>
      ...
      {!scanning && !scanned && !error && (
        <div style={{ textAlign: 'center', paddingTop: 60 }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--text-4)', marginTop: 6, fontFamily: 'IBM Plex Mono, monospace', lineHeight: 1.8 }}>
            {tickers.length} tickers · CSP &amp; CC · Mode: {meta.label}
          </div>
          <div style={{ fontSize: 11, color: meta.color, marginTop: 2, fontFamily: 'IBM Plex Mono, monospace' }}>{meta.sub}</div>
          <button onClick={handleScan} style={{ marginTop: 16, padding: '8px 24px', fontSize: 13, fontWeight: 600, background: 'var(--accent-dim)', border: '1px solid rgba(0,229,255,0.25)', color: 'var(--accent)', cursor: 'pointer', fontFamily: "'Chakra Petch', sans-serif", letterSpacing: '1.5px', textTransform: 'uppercase' }}>
            START SCAN
          </button>
        </div>
      )}
      ...
      {scanned && !scanning && cards.length === 0 && (
        <div style={{ textAlign: 'center', paddingTop: 40 }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--text-5)', marginTop: 6, fontFamily: 'IBM Plex Mono, monospace' }}>{meta.sub}</div>
          <button onClick={() => saveMode('all')} style={{ marginTop: 12, padding: '5px 14px', fontSize: 11, background: 'var(--bg-elevated)', border: '1px solid var(--border)', color: 'var(--text-3)', cursor: 'pointer', fontFamily: "'Chakra Petch', sans-serif" }}>
            SWITCH TO ALL
          </button>
        </div>
      )}
            ...
            return (
              <div key={card.symbol} style={{ width: CARD_W, minWidth: CARD_W, maxWidth: CARD_W, background: 'var(--bg-card)', border: `1px solid ${idx < 3 && hasData ? 'rgba(0,229,255,0.15)' : 'var(--border)'}`, borderRadius: 8, overflow: 'hidden', flexShrink: 0 }}>
                ...
                <div onClick={() => hasData && toggleCollapse(card.symbol)}
                  ...
                  {hasData && <span style={{ fontSize: 9, fontWeight: 700, color: idx < 3 ? 'var(--accent)' : 'var(--text-5)', fontFamily: "'Chakra Petch', sans-serif", minWidth: 16 }}>#{idx + 1}</span>}
                  <span style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: 15, fontWeight: 700, color: idx === 0 && hasData ? 'var(--accent)' : hasData ? 'var(--text-1)' : 'var(--text-4)', letterSpacing: '1px' }}>{card.symbol}</span>
                  {card.price > 0 && <span style={{ fontSize: 11, color: 'var(--text-3)', fontFamily: 'IBM Plex Mono, monospace' }}>${card.price.toFixed(2)}</span>}
                  {shares > 0 && <span style={{ padding: '1px 5px', fontSize: 9, fontWeight: 700, background: '#3b82f615', border: '1px solid #3b82f640', color: '#3b82f6', fontFamily: "'Chakra Petch', sans-serif" }}>{shares} SHR</span>}
                  <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
                    {hasData ? (
                      <>
                        <div style={{ textAlign: 'right' }}>
                          <div style={{ fontSize: 7, color: 'var(--text-4)', letterSpacing: '1px', fontWeight: 600 }}>SCORE</div>
                          <div style={{ fontFamily: "'Chakra Petch', sans-serif", fontSize: 14, fontWeight: 700, color: scoreColor(card.bestScore) }}>{card.bestScore}</div>
                        </div>
                        <div style={{ textAlign: 'right' }}>
                          <div style={{ fontSize: 7, color: 'var(--text-4)', letterSpacing: '1px', fontWeight: 600 }}>IV</div>
                          <div style={{ fontSize: 13, color: 'var(--text-2)', fontFamily: 'IBM Plex Mono, monospace' }}>{card.avgIv.toFixed(0)}%</div>
                        </div>
                        <div style={{ textAlign: 'right' }}>
                          <div style={{ fontSize: 7, color: 'var(--text-4)', letterSpacing: '1px', fontWeight: 600 }}>OPTS</div>
                          <div style={{ fontSize: 13, color: 'var(--text-3)', fontFamily: 'IBM Plex Mono, monospace' }}>{card.totalContracts}</div>
                        </div>
                        {isCollapsed ? <ChevronDown size={14} style={{ color: 'var(--text-4)' }} /> : <ChevronUp size={14} style={{ color: 'var(--text-4)' }} />}
                      </>
                    ) : (
                      <span style={{ fontSize: 10, color: 'var(--text-5)', fontFamily: 'IBM Plex Mono, monospace' }}>NO DATA</span>
                    )}
                  </div>
                </div>
