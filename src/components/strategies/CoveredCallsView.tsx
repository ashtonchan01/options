...
const TODAY_CC = new Date(); TODAY_CC.setHours(0,0,0,0)

function isExpiredCC(t: RawTrade): boolean {
  if (!t.expiry) return false
  const d = parseExpiry(t.expiry)
  return d !== null && d < TODAY_CC
}
...
// ─── Identify covered call trades ─────────────────────────────────────────────
// A covered call trade is an OPT call (C). We show all legs — opens & closes.

function isCCTrade(t: RawTrade) {
  return t.assetClass === 'OPT' && t.putCall === 'C'
}

// ─── Summary strip ────────────────────────────────────────────────────────────

function SummaryStrip({ trades }: { trades: RawTrade[] }) {
  const active  = trades.filter(t => !isExpiredCC(t))
  const expired = trades.filter(t => isExpiredCC(t))
    ...
    <div className="cc-summary-strip">
      {cards.map(c => (
        <div key={c.label} className="cc-summary-card">
          ...
          <div className="stat-value cc-summary-value" style={{ color: c.color ?? 'var(--text-1)' }}>
            {c.value}
          </div>
        </div>
      ))}
...
function FYBar({ allTrades, selectedFY }: { allTrades: RawTrade[]; selectedFY: number | 'all' }) {
  const fys = useMemo(() => {
    const set = new Set(allTrades.map(t => getFY(t.tradeDate)))
  ...
  const fyNetMap = useMemo(() => {
    ...
    for (const fy of fys) {
      ...
      m[fy] = allTrades
        .filter(t => { const d = new Date(t.tradeDate); return d >= from && d <= to })
        ...
        return (
          <div key={fy} className={`cc-fy-bar-item${active ? ' active' : ''}`}>
            ...
            <div className="cc-fy-bar-track">
              <div className="cc-fy-bar-fill" style={{
                width: `${pct}%`,
                background: net >= 0 ? '#10b981' : '#f43f5e',
                opacity: active ? 1 : 0.45,
              }} />
            </div>
            <div className={`cc-fy-bar-val ${pnlCls(net)}`} style={{ opacity: active ? 1 : 0.6 }}>
              {fmt$(net, 0)}
            </div>
          </div>
        )
  ...
  if (trades.length === 0) {
    return (
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-5)', fontSize: 14 }}>
        No covered call trades in this period
      </div>
    )
  }
  ...
  return (
    <div style={{ flex: 1, overflow: 'auto' }}>
      <table className="trade-table">
        ...
        <tbody>
          {trades.map((t, i) => {
            const expired = isExpiredCC(t)
            const sellColor = '#10b981'
            const buyColor  = '#f59e0b'

            return (
              <tr key={i} style={expired ? { opacity: 0.38 } : undefined}>
                <td className="mono" style={{ color: 'var(--text-3)', whiteSpace: 'nowrap', fontSize: 12 }}>
                  {fmtDate(t.tradeDate)}
                </td>
                <td style={{ fontWeight: 700, color: expired ? 'var(--text-4)' : 'var(--text-1)', fontFamily: 'IBM Plex Mono, monospace' }}>
                  {t.underlyingSymbol ?? '—'}
                </td>
                <td style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 11, color: 'var(--text-4)' }}>
                  {t.symbol}
                </td>
                <td className="mono" style={{ textAlign: 'right', color: 'var(--text-3)' }}>
                  {t.strike ? `$${t.strike.toLocaleString()}` : '—'}
                </td>
                <td className="mono" style={{ fontSize: 12, color: expired ? 'var(--text-5)' : 'var(--text-3)' }}>
                  {fmtExpiry(t.expiry ?? '')}
                </td>
                <td style={{ textAlign: 'center' }}>
                  {expired ? (
                    <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 6px', color: 'var(--text-5)', background: 'var(--bg-2)', border: '1px solid var(--border)', letterSpacing: '0.06em' }}>
                      EXPIRED
                    </span>
                  ) : (
                    <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 6px',
                      color: t.quantity < 0 ? sellColor : buyColor,
                      background: t.quantity < 0 ? `${sellColor}14` : `${buyColor}14`,
                      border: `1px solid ${t.quantity < 0 ? sellColor : buyColor}33`,
                ...
                <td className="mono" style={{ textAlign: 'right', color: 'var(--text-3)' }}>
                  {t.tradePrice != null ? `$${t.tradePrice.toFixed(2)}` : '—'}
                </td>
                <td className="mono" style={{ textAlign: 'right', color: '#f59e0b', fontSize: 12 }}>
                  {t.commissions != null ? fmt$(t.commissions, 2) : '—'}
                </td>
                <td className={`mono ${expired ? 'neu' : pnlCls(t.netCash)}`} style={{ textAlign: 'right', fontWeight: 700, color: expired ? 'var(--text-4)' : undefined }}>
                  {fmt$(t.netCash)}
                </td>
              </tr>
            )
...
export default function CoveredCallsView({ state, labelFilter, hasLabels }: Props) {
  ...
  const availableFYs = useMemo(() => {
    const set = new Set(allCC.map(t => getFY(t.tradeDate)))
    return Array.from(set).sort((a, b) => b - a)
  }, [allCC])

  // Filter by FY
  const filtered = useMemo(() => {
    if (selectedFY === 'all') return allCC
    const [from, to] = fyRange(selectedFY)
    return allCC.filter(t => { const d = new Date(t.tradeDate); return d >= from && d <= to })
  }, [allCC, selectedFY])
    ...
    <div className="cc-root">
      ...
      <div className="cc-header">
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span className="cc-title-badge">CC</span>
            <h2 className="cc-title">Covered Calls</h2>
          </div>
          <div className="cc-subtitle">Trade log — all call option legs</div>
        </div>
        ...
        <div className="cc-controls">
          ...
          <div className="cc-control-group">
            ...
            >
              <option value="all">All Time ({allCC.length} trades)</option>
              {availableFYs.map(fy => {
                const [from, to] = fyRange(fy)
                const count = allCC.filter(t => { const d = new Date(t.tradeDate); return d >= from && d <= to }).length
                return (
                  <option key={fy} value={fy}>
                    {fyLabel(fy)} — {count} trade{count !== 1 ? 's' : ''}
                  </option>
                )
              })}
            </select>
      ...
      {availableFYs.length > 1 && (
        <div className="cc-section">
          ...
          <FYBar allTrades={allCC} selectedFY={selectedFY} />
        </div>
      )}
