...
const C = {
  cyan: '#00E5FF', cyanDim: 'rgba(16,185,129,0.12)', cyanGlow: 'rgba(16,185,129,0.06)',
  green: '#10b981', greenDim: 'rgba(0,208,132,0.12)',
  red: '#ef4444', redDim: 'rgba(255,71,87,0.12)',
    ...
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: '100%' }}>
      ...
      {gridLines.map((g, i) => (
        <g key={i}>
          ...
          <text x={PL - 4} y={g.y + 2.5} fill="var(--text-4)" fontSize="6.5" textAnchor="end" fontFamily="Inter, sans-serif">{g.label}</text>
        </g>
      ))}
      ...
      {dateLabels.map((d, i) => (
        <text key={i} x={d.x} y={H - 5} fill="var(--text-4)" fontSize="6.5" textAnchor="middle" fontFamily="Inter, sans-serif">{d.label}</text>
      ))}
      {/* Legend */}
    ...
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: '100%' }}>
      ...
      {gridLines.map((g, i) => (
        <g key={i}>
          ...
          <text x={PL - 4} y={g.y + 3} fill="var(--text-4)" fontSize="7" textAnchor="end" fontFamily="Inter, sans-serif">{g.label}</text>
        </g>
      ))}
      ...
      <text x={PL} y={H - 4} fill="var(--text-4)" fontSize="7" fontFamily="Inter, sans-serif">{points[0].date.slice(2, 7)}</text>
      <text x={W - PR} y={H - 4} fill="var(--text-4)" fontSize="7" textAnchor="end" fontFamily="Inter, sans-serif">{points[points.length - 1].date.slice(2, 7)}</text>
      <text x={W - PR + 4} y={scaleY(finalPnl) + 3} fill={lineColor} fontSize="9" fontWeight="700" fontFamily="Inter, sans-serif">{fmt$(finalPnl)}</text>
    </svg>
  )
}
...
function MonthlyTable({ trades }: { trades: Trade[] }) {
  ...
  const mTd: React.CSSProperties = { ...tdS, textAlign: 'center', padding: '5px 6px', fontFamily: "'Inter', sans-serif", fontSize: 11 }
...
function DayBreakdown({ trades }: { trades: Trade[] }) {
  ...
  const dTd: React.CSSProperties = { ...tdS, textAlign: 'center', padding: '8px 8px', fontFamily: "'Inter', sans-serif", fontSize: 12 }
...
export default function BacktestView({ }: { state: AppState }) {
  ...
  return (
    <div style={{ padding: 14, height: '100%', display: 'flex', flexDirection: 'column', gap: 8, overflow: 'hidden' }}>
      ...
      <div className="backtest-config" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 6, padding: '14px 18px', flexShrink: 0 }}>
        <div className="config-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <div>
            ...
            <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 18, fontWeight: 700, color: 'var(--text-1)', letterSpacing: 1 }}>
              SPX BULL PUT SPREAD
            </div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 10, color: 'var(--text-4)', letterSpacing: 2, marginBottom: 2 }}>DATA RANGE</div>
            <div style={{ fontSize: 12, color: 'var(--text-3)', fontFamily: "'Inter', sans-serif" }}>
            ...
            <div className="backtest-stats" style={{ display: 'grid', gridTemplateColumns: 'repeat(10, 1fr)', gap: 4, flexShrink: 0 }}>
              {([
                ...
                <div key={s.label} style={{
                  ...
                  <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 16, fontWeight: 700, color: s.color, lineHeight: 1.1 }}>{s.value}</div>
                </div>
              ))}
            </div>
                    ...
                    {singleResult.trades.map((t, i) => {
                      ...
                      return (
                        <tr key={i} style={{ background: i % 2 === 0 ? 'transparent' : 'var(--bg-surface)' }}>
                          ...
                          <td style={{ ...tdS, fontFamily: "'Inter', sans-serif", fontSize: 11, color: C.blue }}>{t.holdDays}</td>
                          <td style={{ ...tdS, fontFamily: "'Inter', sans-serif", fontSize: 11 }}>{t.entrySpx.toFixed(0)}</td>
                          <td style={{ ...tdS, fontFamily: "'Inter', sans-serif", fontSize: 11, color: 'var(--text-3)' }}>{t.shortK}/{t.longK}</td>
                          <td style={{ ...tdS, fontFamily: "'Inter', sans-serif", fontSize: 11, color: C.green }}>${t.netCredit.toFixed(2)}</td>
                          <td style={{ ...tdS, fontFamily: "'Inter', sans-serif", fontSize: 11 }}>{t.exitSpx.toFixed(0)}</td>
                          ...
                          <td style={{ ...tdS, fontFamily: "'Inter', sans-serif", fontSize: 11, color: 'var(--text-3)' }}>{t.contracts}</td>
                          <td style={{ ...tdS, fontFamily: "'Inter', sans-serif", fontSize: 11, fontWeight: 700, color: t.pnl >= 0 ? C.green : C.red }}>{fmt$(t.pnl)}</td>
                          <td style={{ ...tdS, fontFamily: "'Inter', sans-serif", fontSize: 11, color: 'var(--text-2)' }}>{fmt$(t.equityAfter)}</td>
                        </tr>
                      )
                    })}
                  </tbody>
      ...
      {activeView === 'single' && singleResult && singleResult.numTrades === 0 && (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ textAlign: 'center', color: 'var(--text-3)', fontSize: 14 }}>
            <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 20, fontWeight: 700, color: 'var(--text-1)', marginBottom: 8 }}>NO TRADES FOUND</div>
                    ...
                    {sortedSweep.map((r, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {COLUMNS.map(c => (
                            <td key={c.key} style={{ ...tdS, color: cellColor(c.key, r[c.key] as number), fontFamily: "'Inter', sans-serif", fontSize: 11 }}>
                  ...
                  <tbody>
                    {selectedSweep.trades.map((t, i) => (
                      <tr key={i} style={{ background: i % 2 === 0 ? 'transparent' : 'var(--bg-surface)' }}>
                        ...
                        <td style={{ ...tdS, fontFamily: "'Inter', sans-serif", fontSize: 11 }}>{t.entrySpx.toFixed(0)}→{t.exitSpx.toFixed(0)}</td>
                        <td style={{ ...tdS, fontFamily: "'Inter', sans-serif", fontSize: 11, color: 'var(--text-3)' }}>{t.shortK}/{t.longK}</td>
                        <td style={{ ...tdS, fontFamily: "'Inter', sans-serif", fontSize: 11, color: C.green }}>{t.netCredit.toFixed(2)}</td>
                        <td style={{ ...tdS, fontFamily: "'Inter', sans-serif", fontSize: 11, color: C.gold }}>{t.exitCost.toFixed(2)}</td>
                        <td style={{ ...tdS, fontFamily: "'Inter', sans-serif", fontSize: 11, fontWeight: 700, color: t.pnl >= 0 ? C.green : C.red }}>{fmt$(t.pnl)}</td>
                        <td style={{ ...tdS, fontFamily: "'Inter', sans-serif", fontSize: 11, color: 'var(--text-4)' }}>{t.holdDays}</td>
      ...
      {activeView === 'none' && !running && (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ textAlign: 'center', maxWidth: 520 }}>
            <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 22, fontWeight: 700, color: 'var(--text-1)', letterSpacing: 1, marginBottom: 8 }}>
      ...
      {running && (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 18, fontWeight: 700, color: C.cyan, animation: 'pulse 1s ease-in-out infinite' }}>COMPUTING…</div>
        </div>
      )}
    </div>
  )
