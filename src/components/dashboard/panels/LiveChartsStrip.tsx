...
export default function LiveChartsStrip({ quotes }: { quotes: Record<string, MarketQuote> }) {
  return (
    <div className="dash-panel">
      ...
      <div style={{ flex: 1, display: 'flex', gap: 10, overflowX: 'auto', padding: '2px 2px 6px' }}>
        {EXCHANGES.map(ex => {
          const q = quotes[ex.symbol]
          const color = !q ? 'var(--text-3)' : q.change >= 0 ? '#10b981' : '#f43f5e'
          return (
            <div key={ex.symbol} style={{
              flexShrink: 0, width: 108, background: 'var(--bg-page)', border: '1px solid var(--border-light)',
