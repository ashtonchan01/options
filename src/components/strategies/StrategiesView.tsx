...
function LegRow({ leg }: { leg: OptionLeg }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontFamily: 'IBM Plex Mono, monospace', color: '#5D6580', textAlign: 'right', fontSize: 12 }}>
...
function StrategyCard({ s }: { s: Strategy }) {
  ...
  return (
    <div style={{ background: '#131726', border: '1px solid #1E2540', borderRadius: 10, borderLeft: `3px solid ${color}`, overflow: 'hidden' }}>
      ...
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 16px', borderBottom: '1px solid #1E2540' }}>
        <span style={{ fontSize: 15, fontWeight: 700, color: '#EAEDF3', fontFamily: 'IBM Plex Mono, monospace', minWidth: 50 }}>
          {s.underlying}
        </span>
        ...
        <span style={{ fontSize: 11, fontFamily: 'IBM Plex Mono, monospace', color: minDte <= 21 ? '#f59e0b' : '#2A3250' }}>
          {minDte}d
        </span>
        <div style={{ flex: 1 }} />
        {s.netPremiumReceived > 0 && (
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 11, color: '#5D6580', letterSpacing: '0.06em' }}>PREM</div>
            <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 14, color: '#5D6580' }}>{fmt$(s.netPremiumReceived)}</div>
          </div>
        )}
        <div style={{ textAlign: 'right', minWidth: 70 }}>
          <div style={{ fontSize: 11, color: '#5D6580', letterSpacing: '0.06em' }}>P&L</div>
      ...
      {/* Shares row */}
      {s.shares && (
        <div style={{ display: 'flex', gap: 20, padding: '9px 16px', borderBottom: '1px solid #1A1F35', fontSize: 13 }}>
          <span style={{ fontFamily: 'IBM Plex Mono, monospace', color: '#2A3250', fontSize: 11, fontWeight: 700, letterSpacing: '0.06em', alignSelf: 'center' }}>STOCK</span>
      ...
      {/* Column headers */}
      <div style={{ display: 'grid', gridTemplateColumns: '110px 1fr 70px 90px 45px 45px 65px', padding: '5px 14px', fontSize: 11, fontWeight: 600, color: '#5D6580', letterSpacing: '0.06em' }}>
      ...
      {/* Progress bar */}
      {pct !== null && (
        <div style={{ padding: '10px 16px', borderTop: '1px solid #1A1F35', display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 11, color: '#5D6580', letterSpacing: '0.06em', minWidth: 80 }}>PROFIT</span>
            ...
            <thead>
              <tr style={{ borderBottom: '1px solid #1E2540' }}>
                {['symbol','assetClass','putCall','strike','expiry','underlyingSymbol','qty','positionValue'].map(h => (
                  <th key={h} style={{ padding: '10px 14px', textAlign: 'left', color: '#9198AE', fontWeight: 700, letterSpacing: '0.06em', fontSize: 12 }}>{h.toUpperCase()}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {state.sync.positions.map((p, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #1A1F35', background: i % 2 ? '#0F1220' : 'transparent' }}>
                  <td style={{ padding: '9px 14px', color: '#EAEDF3' }}>{p.symbol}</td>
                  ...
                  <td style={{ padding: '9px 14px', color: '#9198AE' }}>{p.underlyingSymbol ?? '—'}</td>
                  <td style={{ padding: '9px 14px', color: '#9198AE' }}>{p.quantity}</td>
                  <td style={{ padding: '9px 14px', color: p.positionValue > 0 ? '#10b981' : p.positionValue < 0 ? '#f43f5e' : '#5D6580' }}>{p.positionValue.toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, minHeight: 0 }}>
            ...
            {col1Types.map(t => (
              <div key={t}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                  ...
                  <span style={{ fontSize: 12, color: '#5D6580' }}>{byType[t].length}</span>
        ...
        <div style={tile}>
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: 8, display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            {col2Types.length === 0 && (
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#2A3250', fontSize: 14 }}>
