...
async function getCrumb() {
  ...
  const cookieStr = cookieParts.map(c => c.split(';')[0].trim()).filter(c => c.includes('=')).join('; ')
  ...
  if (!crumb || crumb.startsWith('{') || crumb.length > 40) throw new Error('Invalid crumb')
  cachedCrumb = crumb
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const results = await Promise.all(
      syms.map(s => fetchOne(s, crumb, cookies).catch(() => ({ symbol: s, dates: [] })))
    )
    const data = {}
