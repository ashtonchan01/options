      ...
      {months.map(m => {
        ...
        return (
          ...
          >
            ...
            <div style={{ position: 'absolute', bottom: 4, left: '50%', transform: 'translateX(-50%)', fontSize: 11, color: 'var(--text-3)', fontFamily: 'Inter, sans-serif', whiteSpace: 'nowrap' }}>
              {m.label}
            </div>
          </div>
        )
      ...
      {underlyings.map(sym => {
        ...
        return (
          <div key={sym} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: 'var(--bg-elevated)', borderRadius: 8 }}>
            <span style={{ fontWeight: 700, color: 'var(--text-1)', fontFamily: 'Inter, sans-serif', fontSize: 14, minWidth: 50 }}>
              {sym}
            </span>
            <span style={{
            ...
            <div style={{ textAlign: 'right', flexShrink: 0 }}>
              {stk && (
                <div style={{ fontSize: 12, fontFamily: 'Inter, sans-serif', color: 'var(--text-3)' }}>
                  {stk.quantity} sh
                </div>
              )}
              {active && (
                <div style={{ fontSize: 12, fontFamily: 'Inter, sans-serif', color: active.unrealizedPnL >= 0 ? '#10b981' : '#f43f5e' }}>
              ...
              <tbody>
                {[...months].reverse().map((m, i) => (
                  <tr key={m.key} style={{ borderBottom: '1px solid var(--border-light)', background: i % 2 ? 'var(--bg-page)' : 'transparent' }}>
                    <td style={{ padding: '10px 16px', fontFamily: 'Inter, sans-serif', color: 'var(--text-3)' }}>{m.label}</td>
                    <td style={{ padding: '10px 16px', textAlign: 'right', fontFamily: 'Inter, sans-serif', color: m.optionPnL >= 0 ? '#10b981' : '#f43f5e' }}>{fmt$(m.optionPnL)}</td>
                    <td style={{ padding: '10px 16px', textAlign: 'right', fontFamily: 'Inter, sans-serif', color: m.stockPnL >= 0 ? '#10b981' : '#f43f5e' }}>{fmt$(m.stockPnL)}</td>
                    <td style={{ padding: '10px 16px', textAlign: 'right', fontFamily: 'Inter, sans-serif', fontWeight: 600, color: m.total >= 0 ? '#10b981' : '#f43f5e' }}>{fmt$(m.total)}</td>
                    <td style={{ padding: '10px 16px', textAlign: 'right', fontFamily: 'Inter, sans-serif', color: 'var(--text-5)' }}>{m.tradeCount}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr style={{ borderTop: '1px solid var(--border)' }}>
                  <td style={{ padding: '10px 16px', fontFamily: 'Inter, sans-serif', color: 'var(--text-4)', fontWeight: 700 }}>TOTAL</td>
                  <td style={{ padding: '10px 16px', textAlign: 'right', fontFamily: 'Inter, sans-serif', fontWeight: 600, color: months.reduce((s,m)=>s+m.optionPnL,0) >= 0 ? '#10b981' : '#f43f5e' }}>
                    {fmt$(months.reduce((s,m)=>s+m.optionPnL,0))}
                  </td>
                  <td style={{ padding: '10px 16px', textAlign: 'right', fontFamily: 'Inter, sans-serif', fontWeight: 600, color: months.reduce((s,m)=>s+m.stockPnL,0) >= 0 ? '#10b981' : '#f43f5e' }}>
                    {fmt$(months.reduce((s,m)=>s+m.stockPnL,0))}
                  </td>
                  <td style={{ padding: '10px 16px', textAlign: 'right', fontFamily: 'Inter, sans-serif', fontWeight: 700, color: totalRealizedPnL >= 0 ? '#10b981' : '#f43f5e' }}>
                    {fmt$(totalRealizedPnL)}
                  </td>
                  <td style={{ padding: '10px 16px', textAlign: 'right', fontFamily: 'Inter, sans-serif', color: 'var(--text-4)' }}>
