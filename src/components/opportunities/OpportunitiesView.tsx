...
export default function OpportunitiesView({ state }: Props) {
  ...
  return (
    <div style={{ padding: 20, height: '100%', display: 'flex', flexDirection: 'column', gap: 14, overflow: 'hidden' }}>
      ...
      {/* ── Loading ─────────────────────────────────────────────────────────── */}
      {scanning && (
        <div style={{ color: '#9198AE', fontSize: 15, paddingTop: 40, textAlign: 'center' }}>
          Scanning {tickers.length} tickers{scanProgress ? ` — ${scanProgress}` : '...'}
          <br />
          <span style={{ fontSize: 13, color: '#5D6580', marginTop: 6, display: 'block' }}>
      ...
      {!scanning && !scanned && !error && (
        <div style={{ color: '#9198AE', fontSize: 15, paddingTop: 60, textAlign: 'center' }}>
      ...
      {filtered.length > 0 && (
        <div style={{ background: '#131726', border: '1px solid #1E2540', borderRadius: 10, overflow: 'auto', flex: 1, minHeight: 0 }}>
            ...
            <tbody>
              {filtered.map((r, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #1A1F35', background: i % 2 ? '#0F1220' : 'transparent' }}>
                  <td style={{ padding: '10px 14px 10px 16px', fontWeight: 700, color: '#EAEDF3', fontFamily: 'IBM Plex Mono, monospace', fontSize: 14 }}>
                    {r.underlying}
                  </td>
                  ...
                  <td style={{ padding: '12px 16px', textAlign: 'right', fontFamily: 'IBM Plex Mono, monospace', color: '#EAEDF3' }}>
                    ${r.strike.toLocaleString()}
                  </td>
                  <td style={{ padding: '12px 16px', textAlign: 'right', fontFamily: 'IBM Plex Mono, monospace', color: '#9198AE', fontSize: 14 }}>
                    {fmtExpiry(r.expiry)}
                  </td>
                  <td style={{ padding: '12px 16px', textAlign: 'right', fontFamily: 'IBM Plex Mono, monospace', color: '#9198AE' }}>
                    {r.dte}
                  </td>
                  ...
                  <td style={{ padding: '12px 16px', textAlign: 'right' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'flex-end' }}>
                      ...
                      <span style={{ fontFamily: 'IBM Plex Mono, monospace', color: '#9198AE', fontSize: 13, minWidth: 28, textAlign: 'right' }}>
          ...
          <div style={{ padding: '8px 16px', color: '#5D6580', fontSize: 13, fontFamily: 'IBM Plex Mono, monospace', borderTop: '1px solid #1E2540' }}>
