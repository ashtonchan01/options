...
function statusOf(s: Strategy, actions: Action[]): { label: string; color: string } {
  ...
  if (related?.urgency === 'urgent') return { label: 'URGENT', color: '#ef4444' }
  // WATCH means OTM + expiring soon — goal achieved, show OK not MANAGE
  if (related?.urgency === 'watch') return { label: 'OK', color: '#10b981' }
  ...
  if (pnl < 0 && lossPct > 0.5) return { label: 'URGENT', color: '#ef4444' }
  // Long DTE (>90d) with no big loss = no immediate risk → OK
  if (minDte > 90) return { label: 'OK', color: '#10b981' }
  // High profit + expiring soon = goal achieved → OK (not MANAGE)
  if (premium > 0 && pnl / premium >= 0.75 && minDte <= 7) return { label: 'OK', color: '#10b981' }
  ...
  return (
    <tr style={{ borderBottom: isLast ? 'none' : '1px solid var(--border)' }}>
      ...
      <td style={{ padding: '9px 14px', whiteSpace: 'nowrap' }}>
        <span style={{ fontFamily: 'Inter, sans-serif', fontWeight: 700, fontSize: 13, color: 'var(--text-1)' }}>
          {s.underlying}
        </span>
        {s.shares && (
      ...
      <td style={{ padding: '9px 8px' }}>
        <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: 'var(--text-2)' }}>
          {legLine(s)}
        </span>
      </td>
      ...
      <td style={{ padding: '9px 8px', textAlign: 'right', whiteSpace: 'nowrap' }}>
        {minDte !== null && (
          <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, fontWeight: 600, color: dteColor }}>
            {minDte}d
          </span>
        )}
      </td>
                ...
                <>
                  {credit > 0 && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                      ...
                      <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: '#10b981' }}>
                        {fmt$(credit)}
                      </span>
                    </div>
                  )}
                  ...
                  {s.netPremiumReceived !== 0 && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4, borderTop: '1px solid var(--border)', paddingTop: 2, marginTop: 1 }}>
                      ...
                      <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 11, color: s.netPremiumReceived >= 0 ? '#10b981' : '#ef4444' }}>
      ...
      <td style={{ padding: '9px 14px', textAlign: 'right', whiteSpace: 'nowrap' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 1 }}>
          <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, fontWeight: 600, color: pnlColor(opPnl) }}>
            {fmt$(opPnl)}
          </span>
          {stale && (
            <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.05em', color: '#f59e0b' }}>
              ⚠ STALE — RE-SYNC
            </span>
          )}
          {!stale && status.label === 'URGENT' && (
            <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.06em', color: '#ef4444' }}>
              ITM RISK
            </span>
          )}
        </div>
      ...
      <td style={{ padding: '9px 8px', textAlign: 'center' }}>
        {assignment === 'yes' ? (
          <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', color: '#ef4444', background: '#ef444418', border: '1px solid #ef444440', borderRadius: 3, padding: '2px 7px' }}>
            YES
          </span>
        ) : assignment === 'near' ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
            <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', color: '#f59e0b', background: '#f59e0b18', border: '1px solid #f59e0b40', borderRadius: 3, padding: '2px 7px' }}>
              NEAR
            </span>
            <span style={{ fontSize: 9, color: 'var(--text-4)' }}>&lt;2% from strike</span>
          </div>
        ) : assignment === 'no' ? (
          <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', color: '#10b981', background: '#10b98114', border: '1px solid #10b98135', borderRadius: 3, padding: '2px 7px' }}>
            NO
          </span>
        ) : (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <div style={{ flex: 1, height: 3, background: 'var(--border)', borderRadius: 2, overflow: 'hidden', minWidth: 40 }}>
              <div style={{
                ...
                background: pct >= 0 ? (pct >= 0.5 ? '#10b981' : '#10b981') : '#ef4444',
                borderRadius: 2,
              }} />
            </div>
            <span style={{
  ...
  if (!strategies.length) {
    return (
      <div style={{ padding: '20px 24px' }}>
        <div style={{ color: '#f59e0b', fontWeight: 600, marginBottom: 12, fontSize: 13 }}>
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        {[
          ...
          { label: 'URGENT',        value: String(urgent),            color: urgent > 0 ? '#ef4444' : 'var(--text-4)' },
          ...
          <div key={label} style={{
            ...
            <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 16, fontWeight: 700, color }}>
              {value}
            </div>
          </div>
        ))}
                ...
                <>
                  ...
                  <tr key={`hdr-${type}`} style={{ background: 'var(--bg-elevated)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td colSpan={2} style={{ padding: '5px 14px', textAlign: 'right' }}>
                      <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, fontWeight: 600, color: pnlColor(groupPnl) }}>
                        {fmt$(groupPnl)}
                      </span>
                    </td>
                  </tr>
