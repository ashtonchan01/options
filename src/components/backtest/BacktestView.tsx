...
interface BacktestParams {
  ...
  ticker: string
  startPrice: number
  iv: number           // annualized IV (e.g. 0.45 for 45%)
  delta: number        // target delta for sold options
  dte: number          // days to expiry per cycle
  weeks: number        // total weeks to simulate
  startCapital: number
  sharesPerLot: number // 100 for standard
}
...
function runBacktest(params: BacktestParams): BacktestResult {
  ...
  const r = 0.045
  const T = dte / 365
  const weeklyDrift = 0.0015 // ~8% annual upward drift
  ...
  let maxDrawdown = 0
  let holdingShares = strategy === 'covered_call' // CC starts with shares
  ...
  for (let w = 0; w < weeks; w += Math.ceil(dte / 7)) {
    // Simulate price movement over DTE period
    ...
    if (strategy === 'csp' || (strategy === 'wheel' && !holdingShares)) {
      ...
      if (newPrice > strike) {
        ...
        if (strategy === 'wheel') costBasis = newPrice // reset for next CSP cycle
      } else {
...
export default function BacktestView({ state }: Props) {
  ...
  const [weeks, setWeeks] = useState(104) // 2 years
  const [startCap, setStartCap] = useState(50000)
  const [result, setResult] = useState<BacktestResult | null>(null)

  // Get tickers from portfolio or use defaults
  ...
  const inputStyle: React.CSSProperties = {
    ...
    color: '#EAEDF3', padding: '8px 12px', fontSize: 13,
  ...
  return (
    <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ background: '#131726', border: '1px solid #1E2540', borderRadius: 10, padding: '20px 24px' }}>
        <div style={{ fontSize: 10, color: '#5D6580', letterSpacing: 2, marginBottom: 14, fontWeight: 700 }}>BACKTEST PARAMETERS</div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 12, alignItems: 'end' }}>
          <div>
            <div style={{ fontSize: 9, color: '#5D6580', letterSpacing: 1, marginBottom: 4 }}>TICKER</div>
            <select value={ticker} onChange={e => setTicker(e.target.value)} style={selectStyle}>
              {tickers.map(t => <option key={t} value={t}>{t}</option>)}
            </select>
          </div>
          <div>
            <div style={{ fontSize: 9, color: '#5D6580', letterSpacing: 1, marginBottom: 4 }}>STRATEGY</div>
          ...
          <div>
            <div style={{ fontSize: 9, color: '#5D6580', letterSpacing: 1, marginBottom: 4 }}>DELTA</div>
          ...
          <div>
            <div style={{ fontSize: 9, color: '#5D6580', letterSpacing: 1, marginBottom: 4 }}>DTE</div>
          ...
          <div>
            <div style={{ fontSize: 9, color: '#5D6580', letterSpacing: 1, marginBottom: 4 }}>DURATION</div>
          ...
          <div>
            <div style={{ fontSize: 9, color: '#5D6580', letterSpacing: 1, marginBottom: 4 }}>START CAPITAL</div>
            <input type="number" value={startCap} onChange={e => setStartCap(Number(e.target.value))} style={inputStyle} />
          </div>
          <div style={{ display: 'flex', gap: 6 }}>
            <button onClick={handleRun} style={{
              flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
              padding: '9px 16px', background: '#6366F1', border: 'none', borderRadius: 6,
              color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit',
            }}>
              <Play size={13} /> Run
            </button>
            <button onClick={() => setResult(null)} style={{
              padding: '9px 10px', background: '#131726', border: '1px solid #1E2540', borderRadius: 6,
        ...
        <div style={{ marginTop: 10, fontSize: 10, color: '#2A3250' }}>
          BSM-priced simulation with geometric Brownian motion · IV from typical levels · {delta.toFixed(2)}delta {dte}DTE cycles
        </div>
      </div>
      ...
      {result && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 8 }}>
            {[
              { label: 'FINAL CAPITAL', value: fmt$(result.finalCapital), color: '#EAEDF3' },
              { label: 'TOTAL ROI', value: result.roi.toFixed(1) + '%', color: result.roi >= 0 ? '#10b981' : '#f43f5e' },
              { label: 'WIN RATE', value: result.winRate.toFixed(1) + '%', color: result.winRate >= 70 ? '#10b981' : '#f59e0b' },
              { label: 'MAX DRAWDOWN', value: result.maxDrawdown.toFixed(1) + '%', color: result.maxDrawdown < 15 ? '#10b981' : '#f43f5e' },
              { label: 'TOTAL PREMIUM', value: fmt$(result.totalPremium), color: '#10b981' },
          ...
          {strategy === 'wheel' && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
              {[
                { label: 'ASSIGNED (CSP→SHARES)', value: String(result.assignedCount), color: '#f59e0b' },
                { label: 'CALLED AWAY (CC→CASH)', value: String(result.calledCount), color: '#3b82f6' },
                { label: 'FULL WHEEL CYCLES', value: String(Math.min(result.assignedCount, result.calledCount)), color: '#10b981' },
              ].map(s => (
                <div key={s.label} className="stat-card">
                  <div className="stat-label">{s.label}</div>
          ...
          <div style={{ background: '#131726', border: '1px solid #1E2540', borderRadius: 10, padding: '16px 20px' }}>
            <div style={{ fontSize: 10, color: '#5D6580', letterSpacing: 2, marginBottom: 12, fontWeight: 700 }}>EQUITY CURVE</div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6, fontSize: 9, color: '#5D6580', fontFamily: 'IBM Plex Mono, monospace' }}>
              <span>Week 0</span>
              ...
              <span>Week {result.trades[result.trades.length - 1]?.week ?? 0}</span>
            </div>
          </div>

          {/* Trade log */}
          <div style={{ background: '#131726', border: '1px solid #1E2540', borderRadius: 10, overflow: 'hidden' }}>
            <div style={{ padding: '12px 20px', borderBottom: '1px solid #1E2540', fontSize: 10, color: '#5D6580', letterSpacing: 2, fontWeight: 700 }}>
              TRADE LOG ({result.trades.length} trades)
            </div>
            <table className="trade-table" style={{ fontSize: 12 }}>
              <thead>
                <tr>
                  {['WK', 'ACTION', 'STRIKE', 'STOCK', 'PREMIUM', 'OUTCOME', 'P&L', 'CAPITAL'].map(h => (
                    <th key={h} style={{ padding: '8px 14px', fontSize: 10 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {result.trades.slice(0, 50).map((t, i) => {
                  const outcomeColor = t.outcome === 'win' ? '#10b981' : t.outcome === 'called' ? '#3b82f6' : t.outcome === 'assigned' ? '#f59e0b' : '#f43f5e'
                  return (
                    <tr key={i}>
                      <td className="mono" style={{ padding: '8px 14px', color: '#5D6580' }}>{t.week}</td>
                      <td style={{ padding: '8px 14px' }}>
                        <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 6px', border: `1px solid ${t.action === 'CC' ? '#3b82f620' : '#f43f5e20'}`, color: t.action === 'CC' ? '#818cf8' : '#f43f5e' }}>
                          {t.action}
                        </span>
                      </td>
                      <td className="mono" style={{ padding: '8px 14px', color: '#EAEDF3' }}>{fmt$(t.strike)}</td>
                      <td className="mono" style={{ padding: '8px 14px', color: '#9198AE' }}>{fmt$(t.stockPrice, 2)}</td>
                      <td className="mono" style={{ padding: '8px 14px', color: '#10b981' }}>{fmt$(t.premium)}</td>
                      <td style={{ padding: '8px 14px' }}>
                        <span style={{ fontSize: 10, fontWeight: 700, color: outcomeColor, letterSpacing: 1 }}>{t.outcome.toUpperCase()}</span>
                      </td>
                      <td className="mono" style={{ padding: '8px 14px', color: t.pnl >= 0 ? '#10b981' : '#f43f5e' }}>{fmt$(t.pnl)}</td>
                      <td className="mono" style={{ padding: '8px 14px', color: '#EAEDF3', fontWeight: 600 }}>{fmt$(t.capital)}</td>
                    </tr>
                  )
                })}
              </tbody>
            ...
            {result.trades.length > 50 && (
              <div style={{ padding: '8px 20px', color: '#5D6580', fontSize: 11, borderTop: '1px solid #1E2540' }}>
      ...
      {/* ── Empty state ─────────────────────────────────────────────── */}
      {!result && (
        <div style={{ padding: '60px 0', textAlign: 'center', color: '#5D6580' }}>
          <div style={{ fontSize: 14, marginBottom: 8 }}>Configure parameters above and press <span style={{ color: '#6366F1', fontWeight: 600 }}>Run</span></div>
          <div style={{ fontSize: 12 }}>
            Simulates the wheel strategy (CSP + CC cycles) with BSM-priced options on your tickers
          </div>
        </div>
      )}
    </div>
